...
class mrnn ():
    ...
    def rnn_train(self, x, m, t, f):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            target = tf.compat.v1.placeholder(
                tf.float32, [self.seq_len, None, 1])
            ...
            loss = tf.sqrt(tf.reduce_mean(
                tf.square(mask*outputs - mask*target)))
            ...
            for i in range(self.iteration):
                ...
                _, step_loss = \
                    sess.run([train, loss],
                             feed_dict={mask: np.transpose(np.dstack(m[batch_idx, :, f]), [1, 2, 0]),
            ...
            save_file_name = 'tmp/mrnn_imputation/rnn_feature_' + \
                str(f+1) + '/'
    ...
    def rnn_fc_predict(self, x, m, t):
        ...
        fc_imputed_x = np.reshape(
            fc_imputed_x, [self.no, self.seq_len, self.dim])
        m = np.reshape(m, [self.no, self.seq_len, self.dim])
        x = np.reshape(x, [self.no, self.seq_len, self.dim])
    ...
    def fit(self, x, m, t):
        ...
        for f in range(self.dim):
            ...
            print('Finish ' + str(f+1) +
                  '-th feature training with RNN for imputation')
        # Train FC part
        self.fc_train(x, m, t)
